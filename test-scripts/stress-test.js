 ...
 *
 * Stress tests the fraud evaluation endpoint using multiple merchants.
 * Each iteration logs in as a different merchant and evaluates a transaction.
 *
 * Usage:
 *   node stress-test.js [options]
 *
 * Options:
 *   --iterations, -n    Number of evaluations to execute (default: 50, max: 200)
 ...
 *
 * Examples:
 *   node stress-test.js                  # 50 evaluations, 10 concurrent
 *   node stress-test.js -n 100 -c 20     # 100 evaluations, 20 concurrent
...
Each iteration logs in as a different merchant and evaluates a transaction for fraud.

Usage: node stress-test.js [options]

Options:
  --iterations, -n    Number of evaluations (default: 50, max: ${config.maxMerchants})
...
async function evaluateFraud(merchantIndex) {
    const totalStart = performance.now();
    const { email, cardNumber } = getMerchantCredentials(merchantIndex);
    const amount = randomAmount();
    // Unique IP per merchant to avoid rate limiting
    const clientIp = `10.0.${Math.floor(merchantIndex / 256)}.${merchantIndex % 256}`;
    ...
    try {
        ...
        log(`[#${merchantIndex}] Evaluation request: ${JSON.stringify(evalBody)}`);

        const evalStart = performance.now();
        const response = await fetch(`${config.baseUrl}/fraud/evaluate`, {
            method: 'POST',
            headers: {
        ...
        stats.responseTimes.push(evalTime);
        stats.loginTimes.push(loginTime);
        stats.total++;

        if (response.status === 200) {
            const data = await response.json();
            const fraudStatus = data.data;
            log(`[#${merchantIndex}] SUCCESS - Status: ${fraudStatus}`);

            if (fraudStatus === 'CLEAN') stats.clean++;
            else if (fraudStatus === 'SUSPICIOUS') stats.suspicious++;
            else if (fraudStatus === 'BLOCKED') stats.blocked++;

            const icon = fraudStatus === 'CLEAN' ? '[OK]' : fraudStatus === 'SUSPICIOUS' ? '[!!]' : '[XX]';
            process.stdout.write(`\r${icon} #${merchantIndex} (${email}) login=${formatMs(loginTime)} eval=${formatMs(evalTime)} total=${formatMs(totalTime)} -> ${fraudStatus}          `);
        } else {
            stats.failed++;
            let errorMsg = '';
            try {
            ...
            stats.errors.push({ index: merchantIndex, email, status: response.status, message: errorMsg });
            process.stdout.write(`\r[X] #${merchantIndex} (${email}) failed: ${response.status} eval=${formatMs(evalTime)} total=${formatMs(totalTime)}          `);
        }

        return { success: response.status === 200, evalTime, loginTime, totalTime };
        ...
        stats.errors.push({ index: merchantIndex, email, status: 'ERROR', message: error.message });
        process.stdout.write(`\r[X] #${merchantIndex} error: ${error.message.substring(0, 50)}          `);
...
async function runWithConcurrency(total, concurrency) {
    ...
    async function runNext() {
        ...
        const result = await evaluateFraud(index);
        results.push(result);
        await runNext();
    }
...
function printSummary() {
    ...
    const successRate = stats.total > 0 ? ((stats.clean + stats.suspicious) / stats.total * 100) : 0;

    // Evaluation metrics
    const avgEval = stats.responseTimes.length > 0
        ? stats.responseTimes.reduce((a, b) => a + b, 0) / stats.responseTimes.length : 0;
    const minEval = stats.responseTimes.length > 0 ? Math.min(...stats.responseTimes) : 0;
    const maxEval = stats.responseTimes.length > 0 ? Math.max(...stats.responseTimes) : 0;
    ...
    console.log(`    Target:          POST /fraud/evaluate`);
    ...
    console.log(`    CLEAN:           ${stats.clean} [OK]`);
    console.log(`    SUSPICIOUS:      ${stats.suspicious} [!!]`);
    console.log(`    BLOCKED:         ${stats.blocked} [XX]`);
    console.log(`    Failed:          ${stats.failed} [X]`);
    ...
    console.log('');
    console.log('  Evaluation Performance (excl. login):');
    console.log(`    Avg:             ${formatMs(avgEval)}`);
    console.log(`    Min:             ${formatMs(minEval)}`);
    console.log(`    Max:             ${formatMs(maxEval)}`);
...
main().catch(err => {
    console.error('\n[X] Stress test failed:', err.message);
    process.exit(1);
});
