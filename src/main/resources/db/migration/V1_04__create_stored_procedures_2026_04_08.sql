...
CREATE OR REPLACE FUNCTION sp_merchant_validate_for_create(
    p_email VARCHAR,
    p_phone VARCHAR,
    p_role_id BIGINT
) RETURNS TABLE(email_exists BIGINT, phone_exists BIGINT, role_name VARCHAR) AS $$
...
RETURN QUERY
SELECT
    (SELECT COUNT(*) FROM merchants WHERE email = p_email AND deleted_at IS NULL),
    (SELECT COUNT(*) FROM merchants WHERE phone = p_phone AND deleted_at IS NULL),
    (SELECT r.name FROM roles r WHERE r.id = p_role_id);
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION sp_merchant_validate_for_update(
    p_id BIGINT,
    p_email VARCHAR,
    p_phone VARCHAR
) RETURNS TABLE(email_exists BIGINT, phone_exists BIGINT) AS $$
...
RETURN QUERY
SELECT
    (SELECT COUNT(*) FROM merchants WHERE email = p_email AND deleted_at IS NULL AND id != p_id),
    (SELECT COUNT(*) FROM merchants WHERE phone = p_phone AND deleted_at IS NULL AND id != p_id);
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION sp_merchant_find_by_status(
    p_status TEXT,
    p_sort_field TEXT,
    p_sort_dir TEXT,
    p_limit INT,
...
RETURNS TABLE (
    id BIGINT, address TEXT, kyc_status TEXT, merchant_status TEXT,
    tier TEXT, user_id BIGINT, firstname TEXT, lastname TEXT,
    email TEXT, phone TEXT, user_status TEXT,
    created_at TIMESTAMPTZ, updated_at TIMESTAMPTZ, total_count BIGINT
)
LANGUAGE plpgsql AS $$
BEGIN
RETURN QUERY EXECUTE format(
    ...
    ) USING p_status, p_limit, p_offset;
END;
$$;

CREATE OR REPLACE FUNCTION sp_merchant_find_by_kyc_status(
    p_kyc_status TEXT,
    p_sort_field TEXT,
    p_sort_dir TEXT,
    p_limit INT,
...
RETURN QUERY EXECUTE format(
        'SELECT m.*, u.firstname, u.lastname, u.email, u.phone, u.user_status,
         ...
         FROM merchants m
         JOIN users u ON u.id = m.user_id
         WHERE m.kyc_status = $1 AND m.deleted_at IS NULL
         ORDER BY %I %s
         LIMIT $2 OFFSET $3',
        p_sort_field, p_sort_dir
    ) USING p_kyc_status, p_limit, p_offset;
END;
$$;

CREATE OR REPLACE FUNCTION sp_blacklist_is_actively_blacklisted_by_account(p_account_id BIGINT)
RETURNS BOOLEAN AS $$
BEGIN
RETURN EXISTS(
    SELECT 1
    FROM merchant_blacklist mb
             JOIN accounts a ON a.merchant_id = mb.merchant_id
