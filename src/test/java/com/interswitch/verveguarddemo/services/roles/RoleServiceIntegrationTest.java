...
import com.interswitch.verveguarddemo.repositories.RolePermissionRepository;
...
public class RoleServiceIntegrationTest extends BaseIntegrationTest {
    ...
    void shouldFailCreateRoleWithDuplicateName() {
        ...
        CreateRoleRequest request = new CreateRoleRequest(existing.getName(), PrincipalType.ADMIN);
    ...
    void shouldAssignPermissionToRoleSuccessfully() {
        Role role = roleRepository.findByName(Roles.MERCHANT).orElseThrow();
    ...
    void shouldFailAssignPermissionAlreadyAssigned() {
        Role role = roleRepository.findByName(Roles.SUPER_ADMIN).orElseThrow();
        Permission permission = permissionRepository.findByName(Permissions.PERMISSION_ASSIGN).orElseThrow();
